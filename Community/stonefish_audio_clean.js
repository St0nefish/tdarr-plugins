...
const plugin = (file, librarySettings, inputs, otherArguments) => {
  ...
  const map_channels_idx = new Map();
  const map_audio_idx = new Map();
    ...
    try {
      ...
      if (file.ffProbeData.streams[i].codec_type.toLowerCase() === 'audio') {
        ...
        try {
          ...
          const title = file.ffProbeData.streams[i].tags.title;
          if (inputs.commentary === true
        ...
        try {
          ...
          const channelKey = `${file.ffProbeData.streams[i].tags.language.toLowerCase()}:${file.ffProbeData.streams[i].channels}`;
          if (map_channels_idx.has(channelKey)) {
            // add this index to array stored for this language:channel
            map_channels_idx.get(channelKey)
              .push(i);
          } else {
  ...
  for (const key of map_channels_idx.keys()) {
    const idx_arr = map_channels_idx.get(key);
    if (idx_arr.length > 1) {
      ...
      for (const idx of idx_arr) {
        const thisBitrate = file.ffProbeData.streams[idx].sample_rate;
      ...
      for (const idx of idx_arr) {
        if (idx !== highestBitrateIdx) {
          const audio_idx = map_audio_idx.get(idx);
          audioStreamsRemoved += 1;
